...
/**
 * Release headings — Keep a Changelog brackets or a bare `v` tag:
 * `## [0.8.0] — 2026-06-07` or `## v0.8.0 — 2026-06-07`.
 */
const RELEASE_HEADING =
  /^##\s+(?:\[v?(\d+\.\d+\.\d+[^\s\]]*)\]|v(\d+\.\d+\.\d+[^\s]*))\s+[—-]\s+(\d{4}-\d{2}-\d{2})\s*$/;
/** Upcoming work — skipped by the site parser. */
...
export function parseChangelog(raw: string): ReadonlyArray<ChangelogRelease> {
  ...
  for (const line of raw.replace(/\r\n/g, "\n").split("\n")) {
    ...
    if (heading) {
      ...
      commit();
      const version = heading[1] ?? heading[2]!;
      release = {
        version,
        tag: `v${version}`,
        date: heading[3]!,
