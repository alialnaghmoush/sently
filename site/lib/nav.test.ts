...
describe("headerGeometry", () => {
  test("the home cell matches the hero's column split", async () => {
    // Left pane lives in HeroIntro; the home page only hosts the right column.
    const intro = await Bun.file(
      join(import.meta.dir, "..", "components", "landing", "hero-intro.tsx"),
    ).text();
    expect(intro).toContain(`lg:w-[${headerGeometry("/").paneWidth}]`);
  });
