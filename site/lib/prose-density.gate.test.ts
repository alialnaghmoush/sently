...
const STRUCTURAL =
  /^(<(Cards|Card|Callout|Tabs|Tab|TypeTable|Steps|Step|Accordions?)\b|```|#{1,6} |\||\w+="|[/]?>$)/;
...
/**
 * Recursively list MDX files under a docs folder.
 *
 * @param rel - Path under content/docs
 */
async function listMdxRecursive(rel: string): Promise<string[]> {
  const dir = join(DOCS_CONTENT_DIR, rel);
  const out: string[] = [];
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      out.push(...(await listMdxRecursive(join(rel, entry.name))));
...
describe("docs prose density", () => {
  test(`handbook MDX ≤ ${MAX_CONSECUTIVE} consecutive plain paragraphs`, async () => {
    const files = [
      join(DOCS_CONTENT_DIR, "index.mdx"),
      ...(await listMdx("get-started")),
      ...(await listMdx("channels")),
      ...(await listMdxRecursive("transports")),
      ...(await listMdx("guides")),
